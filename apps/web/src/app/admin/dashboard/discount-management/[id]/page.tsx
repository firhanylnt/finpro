...
const EditDiscount = () => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await api.get(`/discount/${params.id}`);
            formik.setFieldValue("name", data.data.name);
            formik.setFieldValue("coupon", data.data.coupon);
            formik.setFieldValue("discount_type_id", data.data.discount_type_id);
            formik.setFieldValue("product_id", data.data.productdiscount[0].products.id ?? '');
            formik.setFieldValue("store_id", data.data.store_id ?? '');
            formik.setFieldValue("amount", data.data.amount ?? 0);
            formik.setFieldValue("min_purchase", data.data.min_purchase ?? 0);
            formik.setFieldValue("max_discount", data.data.max_discount ?? 0);
            formik.setFieldValue("start_date", new Date(data.data.start_date));
            formik.setFieldValue("end_date", new Date(data.data.end_date));
            ...
            formik.setFieldValue("status", String(data.data.status));
        } catch (error) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            setLoading(true);
            try {
                await api.patch(`/discount/update/${params.id}`, values);
                toast.success('Successfully update discount!');
                setTimeout(() => {
                    router.back();
                }, 3000)
    ...
    return (
        <div className="p-6 w-full mx-auto bg-white shadow-md rounded-lg">
            ...
            <h1 className="text-xl font-bold mb-4">Edit Discount</h1>
