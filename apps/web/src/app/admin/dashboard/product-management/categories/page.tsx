...
const CategoriesListPage = () => {
  ...
  const fetchData = async () => {
    ...
    const res = await api(`/product-category?${query}`);
    const { data } = res.data;
    setCategory(data);
    setLoading(false);
  };
  ...
  return (
    <div className="w-full mx-[30px] mt-[30px]">
      <h1 className="text-2xl mb-6 text-gray-800">Categories</h1>
      <div className="my-4">
      {user?.role === 1 && (
        <Link href={'categories/create'} className="bg-green-500 py-2 px-4 mr-4 rounded-md text-white">+ Create</Link>
      )}
